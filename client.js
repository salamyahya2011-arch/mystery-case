...
const cases = [
  {
    ...
    victim: 'Mr. Kareem',
    goal: 'Find who lied, who went to the Old Well, and who is guilty.',
    suspects: [
  ...
  },
  {
    ...
    victim: 'The Ash Hollow Shrine',
    goal: 'Identify the thief and track the path to the docks.',
    suspects: [
  ...
  },
  {
    ...
    victim: 'Cold Mill Storage',
    goal: 'Find who broke in and why the guard logs are missing.',
    suspects: [
  ...
  },
  {
    ...
    victim: 'Riverstone Inn Ledger',
    goal: 'Recover the ledger and reveal the back hall sighting.',
    suspects: [
  ...
  },
  {
    ...
    victim: 'Night Market Vendor',
    goal: 'Follow the prints and find who took the necklace.',
    suspects: [
...
const elements = {
  ...
  caseIntel: document.getElementById('caseIntel'),
...
function renderHost(state, players) {
  ...
  if (elements.caseFile && state.started) {
    const caseData = getCaseData(state.caseId);
    elements.caseFile.innerHTML = `
      <div class="card"><h3>Case Story</h3><p>${caseData.intro}</p></div>
      <div class="card"><h3>Victim</h3><p>${caseData.victim}</p></div>
      <div class="card"><h3>Goal</h3><p>${caseData.goal}</p></div>
      <div class="card"><h3>Suspects</h3>${caseData.suspects.map(s => `<p><strong>${s.name}</strong> (${s.role}) - ${s.statement}</p>`).join('')}</div>
      <div class="card"><h3>Clues</h3>${caseData.clues.map(c => `<p>${c}</p>`).join('')}</div>
      <div class="card"><h3>Facts</h3>${caseData.facts.map(f => `<p>${f}</p>`).join('')}</div>
    `;
  }

  if (elements.storyLog && state.started) {
    const caseData = getCaseData(state.caseId);
    const unlocked = caseData.chapters.slice(0, state.chapterUnlocked);
    elements.storyLog.innerHTML = unlocked.length
      ? unlocked.map((ch) => `<li><strong>${ch.title}</strong> — ${ch.text.join(' ')}</li>`).join('')
      : '<li>No chapters unlocked yet.</li>';
  }
}
...
function renderPlayer(state) {
  ...
  renderCaseIntel(state, caseData);
...
function renderCaseIntel(state, caseData) {
  if (!elements.caseIntel) return;
  elements.caseIntel.innerHTML = '';

  const unlock = (index) => state.chapterUnlocked >= index;
  const cards = [
    {
      title: 'Victim File',
      unlocked: unlock(1),
      body: `<p>${caseData.victim}</p>`
    },
    {
      title: 'Suspect Dossiers',
      unlocked: unlock(2),
      body: caseData.suspects.map(s => `<p><strong>${s.name}</strong> (${s.role}) - ${s.statement}</p>`).join('')
    },
    {
      title: 'Clue Archive',
      unlocked: unlock(3),
      body: caseData.clues.map(c => `<p>${c}</p>`).join('')
    },
    {
      title: 'Forensic Facts',
      unlocked: unlock(4),
      body: caseData.facts.map(f => `<p>${f}</p>`).join('')
    },
    {
      title: 'Case Goal',
      unlocked: unlock(5) || state.chapterTotal <= 4,
      body: `<p>${caseData.goal}</p>`
    }
  ];
  ...
  cards.forEach((card) => {
    ...
    el.className = 'card';
    if (!card.unlocked) el.classList.add('locked');
    el.innerHTML = `<h3>${card.title}</h3>${card.unlocked ? card.body : '<p>Locked. Progress the story on the main TV.</p>'}`;
    elements.caseIntel.appendChild(el);
  });
}
